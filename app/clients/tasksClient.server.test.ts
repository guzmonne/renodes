...
test("tasksClient.get()", async (assert: Test) => {
  const parent = ulid()
  const userId = ulid()
  const task = new Task({ id: ulid(), content: ulid(), parent, userId })
  ...
  const subTask1 = new Task({ id: ulid(), content: ulid(), parent: task.id, userId })
  const subTask2 = new Task({ id: ulid(), content: ulid(), parent: task.id, userId })
  const subTask3 = new Task({ id: ulid(), content: ulid(), parent: task.id, userId })
...
test("tasksClient.put()", async (assert: Test) => {
  const parent = ulid()
  const userId = ulid()
  const task1 = new Task({ id: ulid(), content: ulid(), parent, userId })
  const task2 = new Task({ id: ulid(), content: ulid(), parent, userId })
  const task3 = new Task({ id: ulid(), content: ulid(), parent, userId })
  assert.deepEqual(await client.put(task1), { data: task1 })
  assert.deepEqual(await client.put(task2), { data: task2 })
  assert.deepEqual(await client.query({ parent, userId }), { data: [task1, task2] })
  assert.deepEqual(await client.put(task3, task1.id), { data: task3 })
  assert.deepEqual(await client.query({ parent, userId }), { data: [task1, task3, task2] })
  assert.end()
})
...
test("tasksClient.update()", async (assert: Test) => {
  ...
  const task = new Task({ id: ulid(), content, parent, userId })
  assert.deepEqual(await client.put(task), { data: task })
  assert.deepEqual(await client.get(task.id, task.userId), { data: task })
  assert.deepEqual(await client.update(task.id, task.set({ content: newContent }), task.userId), {})
...
test("tasksClient.delete()", async (assert: Test) => {
  ...
  const userId = ulid()
  const id = ulid()
  const task = new Task({ id, content: ulid(), parent, userId })
  assert.deepEqual(await client.put(task), { data: task })
  assert.deepEqual(await client.get(task.id, task.userId), { data: task })
  assert.deepEqual(await client.delete(task.id, task.userId), {})
  assert.deepEqual(await client.get(task.id, task.userId), { error: new ModelNotFoundError() })
  assert.end()
})
...
test("tasksClient.after()", async (assert: Test) => {
  ...
  assert.deepEqual(await client.query({ parent, userId }), { data: [task1, task2, task3] })
  assert.deepEqual(await client.after(task3.id, parent, task1.id, userId), {})
  assert.deepEqual(await client.query({ parent, userId }), { data: [task1, task3, task2] })
  assert.deepEqual(await client.after(task2.id, parent, undefined, userId), {})
  assert.deepEqual(await client.query({ parent, userId }), { data: [task2, task1, task3] })
  assert.end()
})
...
test("tasksClient.query()", async (assert: Test) => {
  ...
  const task11 = new Task({ id: ulid(), content: ulid(), parent: task1.id, userId })
  const task12 = new Task({ id: ulid(), content: ulid(), parent: task1.id, userId })
  const task13 = new Task({ id: ulid(), content: ulid(), parent: task1.id, userId })
  ...
  const task31 = new Task({ id: ulid(), content: ulid(), parent: task3.id, userId })
  const task32 = new Task({ id: ulid(), content: ulid(), parent: task3.id, userId })
  const task33 = new Task({ id: ulid(), content: ulid(), parent: task3.id, userId })
  ...
  const task321 = new Task({ id: ulid(), content: ulid(), parent: task32.id, userId })
  const task322 = new Task({ id: ulid(), content: ulid(), parent: task32.id, userId })
  const task323 = new Task({ id: ulid(), content: ulid(), parent: task32.id, userId })
  ...
  // Query recursively
  assert.deepEqual(await client.query({ parent, userId, recursive: true }), { data: [updatedTask1, task2, updatedTask3] })
})
